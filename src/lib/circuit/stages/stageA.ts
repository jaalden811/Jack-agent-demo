...
const stageADefinition: StageDefinition<StageAInput, StageAOutput> = {
  ...
  buildPrompt: (input) => {
    const payload = {
      ...
      task:
        "STAGE A — transcript, people, and evidence extraction. Classify each speaker's side as exactly one of: customer, vendor, partner, unknown (vendor = the internal selling side). Separate customer facts/pain/commitments from vendor questions/recommendations (vendor questions carry ZERO customer intent). Extract organization candidates, stakeholders, and answered vs open questions. Every array item MUST include an evidence_ids array of strings drawn ONLY from the supplied transcript turn ids (t#) and deterministic_evidence ids (use [] when none). Do not invent evidence ids, people, or organizations. Return ONE JSON object with EXACTLY these keys and EXACTLY these item field names: " +
        JSON.stringify({

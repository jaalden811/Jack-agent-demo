...
export function getCanonicalAccount(result: Pick<SecureNetworkingTriageResult, "account_resolution" | "executive_summary">): CanonicalAccount {
  ...
  if (resolution && (resolution.status === "confirmed" || resolution.status === "probable") && resolution.name) {
    return { name: resolution.name, label: resolution.name, status: resolution.status, confidence: resolution.confidence };
  }
  ...
  if (execAccount && execAccount.trim().length > 0) {
    return { name: execAccount, label: execAccount, status: resolution?.status ?? "unresolved", confidence: resolution?.confidence ?? 0 };
  }
  return { name: null, label: RESOLUTION_REQUIRED_LABEL, status: resolution?.status ?? "unresolved", confidence: resolution?.confidence ?? 0 };
}

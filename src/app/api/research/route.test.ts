...
describe("POST /api/research", () => {
  it("creates a run with BuyerTarget shape — no email on buyer fields", async () => {
    const formData = new FormData();
    formData.set("ciscoProduct", "Cisco Meraki");
    formData.set("targetMarket", "mid-market retail");
    formData.set("maxResults", "2");

    const response = await POST(new Request("http://localhost/api/research", { method: "POST", body: formData }));
  ...
  it("returns debug stats on the run", async () => {
    const formData = new FormData();
  ...
  it("returns fallback/unverified run when provider keys are missing", async () => {
    ...
    expect(json.warnings).toEqual(expect.arrayContaining([expect.stringContaining("SEARCH_API_KEY")]));
  });
  ...
  it("reruns a fallback run — new run id, same input", async () => {
    ...
    const createResponse = await POST(new Request("http://localhost/api/research", { method: "POST", body: formData }));
